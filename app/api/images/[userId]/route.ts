...
export async function GET(
  ...
  try {
    ...
    if (session.user.role === 'admin') {
      ...
      isAuthorized = prefix === 'patient' && rows.length > 0 && rows[0].patient_id === Number(actualId);
      ...
      if (doctorRows.length === 0) {
        ...
        if (prefix === 'doctor') {
          ...
          if (!isAuthorized) {
console.log(`Unauthorized Access: Doctor ${doctorId} is attempting to access Patient ${actualId} - Not assigned.`);          }
        } else if (prefix === 'tech') {
          isAuthorized = true;
        }
      }
    ...
    if (!isAuthorized) {
      console.log('Unauthorized - Role:', session.user.role, 'attempting to access:', userId);
