...
// The report content is saved as HTML because we use Tiptap — a rich text editor
// that lets doctors format their text with bold, bullet points, headings, and more.
// But the PDF view needs plain text, so this function converts that HTML into
// clean readable text (e.g. <li>item</li> becomes "• item")
const convertHtmlToFormattedText = (html: string): string => {
  if (!html) return "";

  let text = html;

  // Convert numbered lists to "1. item" format
  ...
  // Convert bullet lists to "• item" format
  ...
  text = text.replace(/<[^>]*>/g, "");
  // Tiptap saves the report as HTML into the database to preserve the doctor's formatting.
  // When showing the report to the patient, we strip that HTML out so the text appears
  // clean and readable instead of showing raw tags
  text = text
    .replace(/&nbsp;/g, " ")
    .replace(/&amp;/g, "&")
    .replace(/&lt;/g, "<")
    ...
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'");
  // Collapse more than 2 blank lines into just 2
  text = text.replace(/\n{3,}/g, "\n\n");
...
const ShowReport = () => {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      try {
        ...
        // Fetch the saved report content
        ...
        if (data.status === "ok" && data.report) {
          ...
          // autosave_text holds the report content as a JSON string
          ...
          // Fill in the report sections, converting any HTML to plain text
          if (autosaveData) {
            setReportData({
          ...
          // If an image was attached to the report, resolve its full URL
          if (report.images) {
            try {
              const imagesData =
        ...
        // Fetch doctor info and build the full URL for their signature image
        try {
  ...
  // Once data is ready, hand everything to PDFPreview in read-only "patient" mode
  return (
    <PDFPreview
